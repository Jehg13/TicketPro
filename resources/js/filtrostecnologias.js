...
if(this.usuarioActualId===null||this.usuarioActualId===undefined){return;}
try{
const clave=`ticketpro_tickets_tomados_${this.usuarioActualId}`;
const guardado=localStorage.getItem(clave);
if(!guardado){this.ticketsTomadosLocal={};return;}
const datos=JSON.parse(guardado);
if(datos&&typeof datos==='object'){this.ticketsTomadosLocal=datos;}
}catch(error){
console.error('Error cargando tickets tomados:',error);
...
try{
const clave=`ticketpro_tickets_tomados_${this.usuarioActualId}`;
localStorage.setItem(clave,JSON.stringify(this.ticketsTomadosLocal));
}catch(error){
console.error('Error guardando tickets tomados:',error);
}
},
registrarTicketTomadoLocal(ticketId,tecnico=null){
if(!ticketId){return;}
...
const id=Number(ticketId);
const registro=this.ticketsTomadosLocal[id];
if(!registro){return false;}
return Number(registro.usuario_id)===Number(this.usuarioActualId);
},
...
const loginActual=String(this.usuarioActualLogin||'').trim().toLowerCase();
if(!loginActual){return false;}
const tomadoPor=datos?.tomado_por;
if(!tomadoPor){return false;}
const loginTomador=typeof tomadoPor==='object'?String(tomadoPor.login||'').trim().toLowerCase():'';
...
if(!this.selectedTicket?.id){console.error('No se encontró el ID del ticket.');return;}
...
if(estadoActual==='solucionado'||estadoActual==='cancelado'){return;}
...
let data={};
try{data=await response.json();}catch(error){console.error('Respuesta JSON inválida:',error);}
if(!response.ok){throw new Error(data.message||`Error HTTP ${response.status}`);}
return data;
}).then(data=>{
if(!data||!data.success){throw new Error(data?.message||'No se pudo tomar el ticket.');}
...
this.registrarTicketTomadoLocal(ticketId,tecnicoNormalizado);
this.tomandoTicket=false;
this.actualizarIconos();
}).catch(error=>{
...
if(typeof tecnico!=='object'){return{id:Number(tecnico),name:'Técnico'};}
const resultado={...tecnico};
if(resultado.foto){
resultado.foto=String(resultado.foto).replace(`${window.location.origin}/storage/`,'').replace('/storage/','').replace(/^storage[\\/]+/,'');
}
return resultado;
},
...
if(!datos.tomado_por&&this.ticketTomadoLocalmente(datos.id)){
const local=this.ticketsTomadosLocal[Number(datos.id)];
if(local){
datos.tomado_por=local.tomado_por||{id:this.usuarioActualId,name:'Técnico'};
datos.estado=datos.estado||'en proceso';
datos.fecha_tomado=datos.fecha_tomado||local.fecha;
}
}
return datos;
},
...
if(estado==='solucionado'||estado==='cancelado'){return false;}
...
if(!form){console.error('No se encontró el formulario.');return;}
...
sincronizarComentariosDespuesDeEnviar(ticketId,requestIdAnterior){
setTimeout(()=>{
...
if(!ticketId){this.comentarios=[];return;}
...
if(requestId!==this._comentariosRequestId){return;}
if(this._comentariosTicketId!==idTicket){return;}
if(!this.openModal){return;}
...
comentarios=comentarios.map(comentario=>this.normalizarComentario(comentario)).filter(Boolean);
comentarios=this.eliminarComentariosDuplicados(comentarios);
...
if(!archivo){this.archivoAdjunto=null;return;}
...
return this.archivoAdjunto.name||'Archivo seleccionado';
},
tamañoArchivoComentario(){
if(!this.archivoAdjunto){return'';}
return this.formatearTamanoArchivo(this.archivoAdjunto.size);
},
...
const indiceSeguro=Math.max(0,Math.min(indice,unidades.length-1));
return parseFloat((bytes/Math.pow(1024,indiceSeguro)).toFixed(2))+' '+unidades[indiceSeguro];
},
...
this.archivoSolucion=archivo;
this.$nextTick(()=>{
this.mostrarPreviewSolucion(archivo);
this.actualizarIconos();
});
},
mostrarPreviewSolucion(archivo){
if(!archivo){return;}
this.limpiarPreviewSolucion();
...
this._previewSolucionUrl=url;
this.$nextTick(()=>{
if(tipo.startsWith('image/')){
const imagen=this.$refs.previewImagenSolucion;
if(!imagen){return;}
imagen.onload=()=>{
if(this._previewSolucionUrl===url){
...
const pdf=this.$refs.previewPdfSolucion;
if(!pdf){return;}
pdf.src=url;
}
});
...
if(this.$refs?.previewImagenSolucion){
this.$refs.previewImagenSolucion.removeAttribute('src');
}
if(this.$refs?.previewPdfSolucion){
this.$refs.previewPdfSolucion.removeAttribute('src');
}
},
...
if(!this.archivoSolucion){return'';}
return this.archivoSolucion.name||'Archivo seleccionado';
},
tamañoArchivoSolucion(){
if(!this.archivoSolucion){return'';}
return this.formatearTamanoArchivo(this.archivoSolucion.size);
},
tipoArchivoSolucion(){
...
const url=evidencia.url_archivo||evidencia.url||evidencia.ruta_url||evidencia.path||evidencia.ruta||(typeof evidencia.archivo==='string'?evidencia.archivo:'');
return{
...evidencia,
url:this.archivoUrl(url),
ruta:evidencia.ruta||evidencia.path||(typeof evidencia.archivo==='string'?evidencia.archivo:''),
nombre:evidencia.nombre||evidencia.nombre_archivo||evidencia.name||evidencia.original_name||this.nombreArchivo(url),
tipo:evidencia.tipo||evidencia.mime_type||evidencia.type||this.tipoArchivo(url),
extension:evidencia.extension||this.extensionArchivo(evidencia.nombre||evidencia.nombre_archivo||evidencia.name||evidencia.original_name||url)
};
}
return null;
},
...
return this.normalizarListaEvidencias(parsed);
}catch(error){
...
if(typeof evidencias==='object'){return this.normalizarListaEvidencias(Object.values(evidencias));}
return[];
},
...
try{
const parsed=JSON.parse(solucion);
return this.obtenerEvidenciasSolucion(parsed);
}catch(error){return[];}
}
...
archivoUrl(archivo){
if(!archivo){return'#';}
if(typeof File!=='undefined'&&archivo instanceof File){return URL.createObjectURL(archivo);}
...
try{return decodeURIComponent(url.split('/').pop().split('?')[0]);}catch(error){return url.split('/').pop().split('?')[0]||'Archivo';}
},
...
const tieneSolucion=!!this.ticketSolucion?.solucion;
...
if(typeof solucion==='string'){
try{solucion=JSON.parse(solucion);}catch(error){solucion={solucion:solucion};}
}
...
const r=imagen.data[i];
const g=imagen.data[i+1];
const b=imagen.data[i+2];
...
return tipo.startsWith('image/')||tipo==='application/pdf'||tipo==='video/mp4'||tipo==='video/webm';
});
if(archivosValidos.length!==archivos.length){this.mostrarError('Algunos archivos no son válidos. Solo puedes seleccionar imágenes, PDF o videos.');}
...
if(!this.solucionForm.solucion||!this.solucionForm.solucion.trim()){
...
this.evidenciasSolucion.filter(archivo=>typeof File!=='undefined'&&archivo instanceof File).forEach(archivo=>{formData.append('evidencias[]',archivo);});
if(this.archivoSolucion&&typeof File!=='undefined'&&this.archivoSolucion instanceof File){
formData.append('evidencia_solucion',this.archivoSolucion);
}
fetch(`/tecnologias/tickets/${this.ticketSolucion.id}/solucion`,{
method:'POST',
headers:{
'X-CSRF-TOKEN':csrfToken,
'Accept':'application/json',
...
if(!data||!data.success){throw new Error(data?.message||'No se pudo guardar la solución.');}
let solucionGuardada=data.solucion||data.data||{};
if(typeof solucionGuardada==='string'){
try{solucionGuardada=JSON.parse(solucionGuardada);}catch(error){solucionGuardada={solucion:this.solucionForm.solucion.trim()};}
}
...
if(texto!==''){url.searchParams.set('buscar',texto);}else{url.searchParams.delete('buscar');}
url.searchParams.delete('page');
