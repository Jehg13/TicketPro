    ...
    <aside
        ...
        <div>
            <div class="flex items-center gap-2 mb-10">
                <span class="text-3xl font-extrabold tracking-wide text-white">Ticket<span
        ...
        <div class="max-w-[1400px] mx-auto">
            ...
            <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    ...
                    <p class="text-sm text-slate-400 mt-1">Consulta y administra la información de los usuarios del
                        sistema</p>
                </div>
                ...
                <div class="relative" x-data="{ perfilAbierto: false }">
                    <button type="button" @click="perfilAbierto=!perfilAbierto"
                        ...
                        <div class="text-left leading-tight hidden sm:block">
                            ...
                            <p class="text-[10px] text-blue-400 font-medium">{{ auth()->user()->role ?? 'Sin rol' }}
                            </p>
                        </div>
                    ...
                    <div x-cloak x-show="perfilAbierto" @click.outside="perfilAbierto=false" x-transition
            ...
            <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4 mb-6">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <div
                        class="flex flex-wrap items-center gap-1 bg-[#0b1026] p-1.5 rounded-xl border border-slate-800">

                        <a href="{{ route('usuarios.tecnologias', array_filter(['buscar' => request('buscar')])) }}"
                            class="px-4 py-2 rounded-lg text-xs font-semibold {{ !request('estado') && !request('departamento') ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800/60' }} transition-all">
                            Todos
                        </a>

                        <a href="{{ route('usuarios.tecnologias', array_filter(['buscar' => request('buscar'), 'estado' => 'activos', 'departamento' => request('departamento')])) }}"
                            class="px-4 py-2 rounded-lg text-xs font-semibold {{ request('estado') === 'activos' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800/60' }} transition-all">
                            Activos
                        </a>

                        <a href="{{ route('usuarios.tecnologias', array_filter(['buscar' => request('buscar'), 'estado' => 'inactivos', 'departamento' => request('departamento')])) }}"
                            class="px-4 py-2 rounded-lg text-xs font-semibold {{ request('estado') === 'inactivos' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800/60' }} transition-all">
                            Inactivos
                        </a>
                    ...
                    <div class="relative">
                        <select onchange="window.location.href=this.value"
                            class="appearance-none bg-[#0b1026] border border-slate-800 text-slate-300 text-xs rounded-xl pl-4 pr-10 py-3 focus:outline-none focus:border-blue-500 min-w-[220px] cursor-pointer">

                            <option
                                value="{{ route('usuarios.tecnologias', array_filter(['buscar' => request('buscar'), 'estado' => request('estado')])) }}"
                                {{ !request('departamento') ? 'selected' : '' }}>
                                Todos los departamentos
                            </option>

                            @foreach ($departamentos as $departamento)
                                <option
                                    value="{{ route('usuarios.tecnologias', array_filter(['buscar' => request('buscar'), 'estado' => request('estado'), 'departamento' => $departamento])) }}"
                                    {{ request('departamento') === $departamento ? 'selected' : '' }}>
                                    {{ $departamento }}
                                </option>
                            @endforeach
                        ...
                        <i data-lucide="chevron-down"
                            class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500"></i>
                    </div>

                </div>

                <form method="GET" action="{{ route('usuarios.tecnologias') }}" class="relative w-full xl:w-72"
                    x-data="{ buscar: @js(request('buscar', '')) }" x-ref="form">
                    ...
                    <input type="text" name="buscar" x-model="buscar" x-ref="input"
                        value="{{ request('buscar') }}" placeholder="Buscar usuario..."
                        class="w-full bg-[#0b1026] border border-slate-800 text-slate-200 text-xs rounded-xl pl-10 pr-10 py-2.5 focus:outline-none focus:border-blue-500">

                    <button type="button" x-show="buscar.length > 0" x-cloak
                        @click="buscar='';$nextTick(()=>{$refs.form.submit()})"
                        class="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center rounded-full text-slate-500 hover:text-white hover:bg-slate-700 transition">
            ...
            <div x-data="usuariosPanel()" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <tbody class="divide-y divide-slate-800/50 text-slate-300">
                                ...
                                @forelse($usuarios as $usuario)
                                    <tr class="hover:bg-slate-800/20 transition"
                                        :class="usuarioSeleccionado && usuarioSeleccionado.login ===
                                            @js($usuario->login) ? 'bg-blue-500/5' : ''">

                                        <td class="py-4">
                                            <div class="flex items-center gap-3">
                                                <img src="{{ $usuario->picture ? asset('storage/' . $usuario->picture) : asset('storage/profile-photos/user.png') }}"
                                                    class="w-9 h-9 rounded-full object-cover ring-2 ring-blue-500/20">
                                                <div class="min-w-0">
                                                    <p class="text-white font-medium truncate">{{ $usuario->name }}
                                                    </p>
                                        ...
                                        <td class="py-4">
                                            ...
                                            @if ($rol === 'gerente ti')
                                                <span
                                                    class="px-2.5 py-1 rounded-full text-[10px] font-medium bg-purple-500/10 text-purple-400 border border-purple-500/20">Gerente
                                                    TI</span>
                                            @elseif(in_array($rol, ['tecnico', 'técnico']))
                                                <span
                                                    class="px-2.5 py-1 rounded-full text-[10px] font-medium bg-blue-500/10 text-blue-400 border border-blue-500/20">Técnico</span>
                                            @else
                                                <span
                                                    class="px-2.5 py-1 rounded-full text-[10px] font-medium bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">{{ $usuario->role ?: 'Usuario' }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="py-4 text-slate-400">
                                            {{ $usuario->departamento ?: 'Sin departamento' }}</td>

                                        <td class="py-4">
                                            <div class="flex items-center justify-center gap-2">

                                                <button type="button" title="Ver usuario"
                                                    @click.prevent="seleccionarUsuario(@js($usuario->login))"
                                                    class="w-8 h-8 rounded-lg flex items-center justify-center bg-slate-800/70 text-slate-400 hover:bg-blue-500/10 hover:text-blue-400 border border-slate-700/50 transition-all">
                                                    <i data-lucide="eye" class="w-4 h-4 pointer-events-none"></i>
                                                </button>

                                                <button type="button" title="Editar usuario"
                                                    @click.prevent="abrirModalEditar(@js($usuario->login))"
                                                    class="w-8 h-8 rounded-lg flex items-center justify-center bg-slate-800/70 text-slate-400 hover:bg-amber-500/10 hover:text-amber-400 border border-slate-700/50 transition-all">
                                                    <i data-lucide="pencil" class="w-4 h-4 pointer-events-none"></i>
                                                </button>

                                                <button type="button" title="Eliminar usuario"
                                                    @click.prevent="abrirModalEliminar(@js($usuario->login),@js($usuario->name))"
                                                    class="w-8 h-8 rounded-lg flex items-center justify-center bg-slate-800/70 text-slate-400 hover:bg-rose-500/10 hover:text-rose-400 border border-slate-700/50 transition-all">
                                ...
                                @empty
                                    ...
                                    <tr>
                                        <td colspan="5" class="py-12 text-center text-slate-500">No se encontraron
                                            usuarios.</td>
                                    </tr>
                                @endforelse
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            @foreach ($usuarios->getUrlRange(max(1, $usuarios->currentPage() - 2), min($usuarios->lastPage(), $usuarios->currentPage() + 2)) as $page => $url)
                                <a href="{{ $url }}"
                                    class="w-8 h-8 rounded-lg text-xs flex items-center justify-center {{ $page == $usuarios->currentPage() ? 'bg-blue-600 text-white font-bold' : 'bg-slate-800 text-slate-300 hover:bg-slate-700' }}">
                                    {{ $page }}
                                </a>
                            @endforeach
                ...
                <div class="bg-[#0b1026]/90 border border-blue-900/40 rounded-2xl p-5 shadow-xl backdrop-blur-md">
                    ...
                    <template x-if="cargando">
                        <div class="h-full min-h-[500px] flex flex-col items-center justify-center text-center">
                            <div
                                class="w-12 h-12 rounded-full border-4 border-slate-700 border-t-blue-500 animate-spin mb-4">
                            </div>
                    ...
                    <template x-if="!usuarioSeleccionado&&!cargando">
                        <div class="h-full min-h-[500px] flex flex-col items-center justify-center text-center">
                            ...
                            <p class="text-xs text-slate-500 mt-2 max-w-xs">Selecciona un usuario de la lista para
                                consultar su información.</p>
                        </div>
                    </template>
                    ...
                    <template x-if="usuarioSeleccionado&&!cargando">
                        <div class="space-y-6">
                            ...
                            <div class="flex items-start justify-between pb-4 border-b border-slate-800">
                                ...
                                <span x-show="usuarioSeleccionado.active"
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>Activa
                                </span>
                            ...
                            <div class="flex flex-col items-center text-center">
                                <div class="relative">
                                    <img :src="usuarioSeleccionado.picture"
                                        class="w-24 h-24 rounded-full object-cover ring-4 ring-blue-500/10 border border-slate-700">
                                    <div class="absolute bottom-1 right-1 w-6 h-6 rounded-full border-4 border-[#0b1026]"
                                        :class="usuarioSeleccionado.active ? 'bg-emerald-500' : 'bg-rose-500'"></div>
                                </div>
                                ...
                                <span
                                    ...
                                    x-text="usuarioSeleccionado.role"></span>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex items-center gap-3 bg-slate-800/40 rounded-xl p-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-white font-medium"
                                                x-text="usuarioSeleccionado.login"></p>
                                        </div>
                                    </div>

                                    <div class="flex items-center gap-3 bg-slate-800/40 rounded-xl p-3">
                                        <div
                                            class="w-8 h-8 rounded-lg bg-amber-500/10 text-amber-400 flex items-center justify-center">
                                            <i data-lucide="contact" class="w-4 h-4"></i>
                                        </div>
                                        <div>
                                            <p class="text-[10px] text-slate-500">Número de empleado</p>
                                            <p class="text-xs text-white font-medium"
                                                x-text="usuarioSeleccionado.numero_empleado||'Sin número de empleado'">
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-3 bg-slate-800/40 rounded-xl p-3">
                                        <div
                                            class="w-8 h-8 rounded-lg bg-cyan-500/10 text-cyan-400 flex items-center justify-center">
                                            <i data-lucide="building" class="w-4 h-4"></i>
                                        </div>
                                        <div>
                                            <p class="text-[10px] text-slate-500">Empresa</p>
                                            <p class="text-xs text-white font-medium"
                                                x-text="usuarioSeleccionado.empresa||'Sin empresa'"></p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-3 bg-slate-800/40 rounded-xl p-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-white font-medium"
                                                x-text="usuarioSeleccionado.oficina||'Sin oficina'"></p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-3 bg-slate-800/40 rounded-xl p-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-white font-medium"
                                                x-text="usuarioSeleccionado.departamento||'Sin departamento'"></p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-3 bg-slate-800/40 rounded-xl p-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-white font-medium"
                                                x-text="usuarioSeleccionado.role||'Usuario'"></p>
                                        </div>
                                    </div>
                                ...
                                <div class="bg-slate-900/60 border border-slate-800 rounded-xl p-3 space-y-3">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-300"
                                                x-text="usuarioSeleccionado.email||'Sin correo'"></p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-300"
                                                x-text="usuarioSeleccionado.phone||'Sin teléfono'"></p>
                                        </div>
                                    </div>
                            ...
                            <div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    <template x-if="usuarioSeleccionado.priv_admin">
                                        <span
                                            class="px-2.5 py-1 rounded-lg bg-blue-500/10 border border-blue-500/20 text-[10px] text-blue-400">Administrador</span>
                                    </template>

                                    <span
                                        class="px-2.5 py-1 rounded-lg bg-blue-500/10 border border-blue-500/20 text-[10px] text-blue-400">Tickets</span>
                                    <span
                                        class="px-2.5 py-1 rounded-lg bg-blue-500/10 border border-blue-500/20 text-[10px] text-blue-400">Comentarios</span>
                ...
                <template x-teleport="body">
                    <div x-cloak x-show="modalEditar" @keydown.escape.window="modalEditar&&cerrarModalEditar()"
                        x-transition.opacity class="fixed inset-0 z-[999999] flex items-center justify-center p-4">

                        <div class="absolute inset-0 bg-black/70 backdrop-blur-sm" @click="cerrarModalEditar()"></div>

                        <div x-show="modalEditar" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 scale-95"
                            x-transition:enter-end="opacity-100 scale-100"
                            x-transition:leave="transition ease-in duration-150"
                            x-transition:leave-start="opacity-100 scale-100"
                            x-transition:leave-end="opacity-0 scale-95" @click.stop
                            class="relative w-full max-w-4xl max-h-[92vh] overflow-hidden bg-[#0b1026] border border-slate-800 rounded-2xl shadow-2xl flex flex-col">
                            ...
                            <div class="p-6 border-b border-slate-800 shrink-0">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div
                                            class="w-11 h-11 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center">
                                            <i data-lucide="pencil" class="w-5 h-5 text-amber-400"></i>
                                        </div>

                                        <div>
                                            <h3 class="text-lg font-bold text-white">Editar usuario</h3>
                                            <p class="text-xs text-slate-500 mt-1">Modifica la información de la
                                                cuenta.</p>
                                        </div>

                                    </div>

                                    <button type="button" @click="cerrarModalEditar()" :disabled="editarCargando"
                                        class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 transition">
                            ...
                            <form @submit.prevent="solicitarPasswordEdicion()" class="flex flex-col min-h-0">
                                ...
                                <div class="p-6 space-y-5 overflow-y-auto">
                                    ...
                                    <div x-show="errorEditar"
                                        class="flex items-center gap-3 p-3 rounded-xl bg-rose-500/10 border border-rose-500/20">
                                    ...
                                    <div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                <label
                                                    class="block text-xs font-medium text-slate-400 mb-2">Nombre</label>
                                                <input type="text" x-model="editarForm.name" required
                                            ...
                                            <div>
                                                <label class="block text-xs font-medium text-slate-400 mb-2">Correo
                                                    electrónico</label>
                                                <input type="email" x-model="editarForm.email" required
                                                    class="w-full bg-slate-900/70 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500">
                                            </div>
                                            ...
                                            <div>
                                                <label
                                                    class="block text-xs font-medium text-slate-400 mb-2">Contraseña</label>
                                                <div class="relative">
                                                    <i data-lucide="lock"
                                    ...
                                    <div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                <label class="block text-xs font-medium text-slate-400 mb-2">Ubicación
                                                    / Oficina</label>

                                                <div class="relative">

                                                    <i data-lucide="map-pin"
                                                        class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-purple-500"></i>

                                                    <select x-model="editarForm.oficina_id"
                                                        @change="actualizarEmpresaPorOficina()" required
                                                        :disabled="cargandoOficinas"
                                                        class="w-full bg-slate-900/70 border border-slate-700 rounded-xl pl-10 pr-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500 disabled:opacity-50">

                                                        <option value="">Selecciona una ubicación</option>

                                                        <template x-for="oficina in oficinas" :key="oficina.id">
                                                            <option :value="String(oficina.id)"
                                                                x-text="oficina.nombre"></option>
                                                        </template>

                                                    </select>
                                                </div>

                                                <p x-show="cargandoOficinas" class="text-[10px] text-slate-500 mt-1">
                                                    Cargando ubicaciones...</p>
                                            </div>

                                            <div>
                                                <label
                                                    class="block text-xs font-medium text-slate-400 mb-2">Departamento</label>

                                                <input type="text" x-model="editarForm.departamento"
                                                    placeholder="Escribe el departamento del usuario"
                                                    class="w-full bg-slate-900/70 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-blue-500">

                                                <p class="text-[10px] text-slate-500 mt-1">El departamento se guarda
                                                    directamente como texto.</p>
                                            </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <div>
                                                <label
                                                    class="block text-xs font-medium text-slate-400 mb-2">Role</label>
                                                <input type="text" x-model="editarForm.role" required
                                            ...
                                            <div>
                                                <label
                                                    class="block text-xs font-medium text-slate-400 mb-2">Estado</label>
                                                <select x-model="editarForm.active" required
                                                    class="w-full bg-slate-900/70 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500">
                                            ...
                                            <div>
                                                <label class="block text-xs font-medium text-slate-400 mb-2">Permiso
                                                    administrador</label>
                                                <select x-model="editarForm.priv_admin" required
                                                    class="w-full bg-slate-900/70 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500">
                                ...
                                <div
                                    class="px-6 py-4 bg-slate-950/50 border-t border-slate-800 flex items-center justify-end gap-3 shrink-0">

                                    <button type="button" @click="cerrarModalEditar()" :disabled="editarCargando"
                                        class="px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-300 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 transition">
                                        Cancelar
                                    </button>

                                    <button type="submit" :disabled="editarCargando"
                                        class="min-w-[130px] px-4 py-2.5 rounded-xl text-xs font-semibold text-white bg-blue-600 hover:bg-blue-500 disabled:opacity-50 transition flex items-center justify-center gap-2">
                                        ...
                                        <template x-if="editarCargando">
                                            <span class="flex items-center gap-2">
                                                <span
                                                    class="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin"></span>Guardando...
                                            </span>
                                        </template>
                ...
                <template x-teleport="body">
                    <div x-cloak x-show="modalConfirmarEdicion"
                        @keydown.escape.window="modalConfirmarEdicion&&cerrarConfirmacionEdicion()"
                        x-transition.opacity class="fixed inset-0 z-[1000001] flex items-center justify-center p-4">

                        <div class="absolute inset-0 bg-black/75 backdrop-blur-sm"
                            @click="cerrarConfirmacionEdicion()"></div>

                        <div x-show="modalConfirmarEdicion" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 scale-95"
                            x-transition:enter-end="opacity-100 scale-100"
                            x-transition:leave="transition ease-in duration-150"
                            x-transition:leave-start="opacity-100 scale-100"
                            x-transition:leave-end="opacity-0 scale-95" @click.stop
                            class="relative w-full max-w-md bg-[#0b1026] border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
                            ...
                            <div class="p-6">
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-slate-400 mt-1">Por seguridad, ingresa tu contraseña
                                            para aplicar los cambios al usuario.</p>
                                    </div>

                                </div>

                                <div class="mt-5 p-4 rounded-xl bg-slate-900/70 border border-slate-800">
                                    <p class="text-[10px] text-slate-500 uppercase tracking-wider">Usuario que será
                                        modificado</p>
                                    <p class="mt-1 text-sm font-semibold text-white truncate"
                                        x-text="editarForm.name"></p>
                                ...
                                <div class="mt-5">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="password" x-model="passwordConfirmarEdicion"
                                            @keydown.enter.prevent="actualizarUsuario()"
                            ...
                            <div
                                class="px-6 py-4 bg-slate-950/50 border-t border-slate-800 flex items-center justify-end gap-3">

                                <button type="button" @click="cerrarConfirmacionEdicion()"
                                    :disabled="editarCargando"
                                    class="px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-300 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 transition">
                                    Atrás
                                </button>

                                <button type="button" @click="actualizarUsuario()"
                                    :disabled="editarCargando || !passwordConfirmarEdicion"
                                    class="min-w-[125px] px-4 py-2.5 rounded-xl text-xs font-semibold text-white bg-blue-600 hover:bg-blue-500 disabled:opacity-50 transition flex items-center justify-center gap-2">
                                    ...
                                    <template x-if="editarCargando">
                                        <span class="flex items-center gap-2">
                                            <span
                                                class="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin"></span>Verificando...
                                        </span>
                                    </template>
                ...
                <template x-teleport="body">
                    <div x-cloak x-show="modalEliminar" @keydown.escape.window="modalEliminar&&cerrarModalEliminar()"
                        x-transition.opacity class="fixed inset-0 z-[1000000] flex items-center justify-center p-4">

                        <div class="absolute inset-0 bg-black/70 backdrop-blur-sm" @click="cerrarModalEliminar()">
                        </div>

                        <div x-show="modalEliminar" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 scale-95"
                            x-transition:enter-end="opacity-100 scale-100"
                            x-transition:leave="transition ease-in duration-150"
                            x-transition:leave-start="opacity-100 scale-100"
                                ...
                                <div>
                                    ...
                                    <div class="p-6">
                                        ...
                                        <div class="flex items-start gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-slate-400 mt-1">¿Estás seguro de que deseas
                                        ...
                                        <div class="mt-5 p-4 rounded-xl bg-slate-900/70 border border-slate-800">
                                            <p class="text-[10px] text-slate-500 uppercase tracking-wider">Usuario
                                                seleccionado</p>
                                            <p class="mt-1 text-sm font-semibold text-white truncate"
                                                x-text="usuarioEliminarNombre"></p>
                                            <p class="text-xs text-slate-500 mt-1" x-text="usuarioEliminarLogin"></p>
                                        </div>

                                        <div
                                            class="mt-4 flex gap-3 p-3 rounded-xl bg-amber-500/5 border border-amber-500/10">
                                            <i data-lucide="info" class="w-4 h-4 text-amber-400 shrink-0 mt-0.5"></i>
                                            <p class="text-xs text-slate-400 leading-relaxed">Esta acción eliminará la
                                                cuenta del usuario. <span class="text-amber-400 font-medium">Esta
                                    ...
                                    <div
                                        ...
                                        <button type="button" @click="cerrarModalEliminar()"
                                            class="px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-300 bg-slate-800 hover:bg-slate-700 transition">
                                            Cancelar
                                        </button>

                                        <button type="button" @click="mostrarConfirmacionPassword()"
                                            class="min-w-[110px] px-4 py-2.5 rounded-xl text-xs font-semibold text-white bg-rose-600 hover:bg-rose-500 transition flex items-center justify-center gap-2">
                            ...
                            <template x-if="mostrarPasswordEliminar">
                                ...
                                <div>
                                    ...
                                    <div class="p-6">
                                        ...
                                        <div class="flex items-start gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-slate-400 mt-1">Ingresa tu contraseña para
                                                    confirmar la eliminación.</p>
                                            </div>

                                        </div>

                                        <div class="mt-5 p-4 rounded-xl bg-slate-900/70 border border-slate-800">
                                            <p class="text-[10px] text-slate-500 uppercase tracking-wider">Usuario que
                                                será eliminado</p>
                                            <p class="mt-1 text-sm font-semibold text-white truncate"
                                                x-text="usuarioEliminarNombre"></p>
                                            <p class="text-xs text-slate-500 mt-1" x-text="usuarioEliminarLogin"></p>
                                        ...
                                        <div class="mt-5">
                                            ...
                                            <label class="block text-xs font-medium text-slate-400 mb-2">Tu
                                                contraseña</label>
                                            ...
                                            <div class="relative">
                                                ...
                                                <input type="password" x-model="passwordEliminar"
                                                    @keydown.enter.prevent="eliminarUsuario()"
                                    ...
                                    <div
                                        ...
                                        <button type="button" @click="volverConfirmacionEliminar()"
                                            :disabled="eliminarCargando"
                                            class="px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-300 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 transition">
                                            Atrás
                                        </button>

                                        <button type="button" @click="eliminarUsuario()"
                                            :disabled="eliminarCargando || !passwordEliminar"
                                            class="min-w-[125px] px-4 py-2.5 rounded-xl text-xs font-semibold text-white bg-rose-600 hover:bg-rose-500 disabled:opacity-50 transition flex items-center justify-center gap-2">
        ...
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('usuariosPanel', () => ({
                        ...
                        .then(data => {
                            ...
                            this.editarForm = {
                                ...
                                active: String(usuario.active ?? '').toUpperCase() === 'Y' ?
                                    'Y' : 'N',
                                priv_admin: String(usuario.priv_admin ?? '').toUpperCase() ===
                                    'Y' ? 'Y' : 'N',
                ...
                solicitarPasswordEdicion() {
                    ...
                    const oficina = this.oficinas.find(item => String(item.id) === String(this
                        .editarForm.oficina_id));
                    ...
                    this.$nextTick(() => {
                        ...
                        const input = document.querySelector(
                            '[x-model="passwordConfirmarEdicion"]');
                ...
                actualizarUsuario() {
                    ...
                    fetch('/tecnologias/usuarios/' + encodeURIComponent(login), {
                        ...
                        .then(async response => {
                            ...
                            try {
                                ...
                                throw new Error(response.status === 500 ?
                                    'Error interno del servidor. Revisa el log de Laravel.' :
                                    `Error HTTP ${response.status}`);
                            ...
                            if (!response.ok) {
                                ...
                                if (data.errors) {
                                    ...
                                    throw new Error(primerError || data.message ||
                                        `Error HTTP ${response.status}`);
                        ...
                        .then(data => {
                            ...
                            if (usuarioActualizado && this.usuarioSeleccionado && this
                        ...
                        .catch(error => {
                            ...
                            this.errorConfirmarEdicion = error.message ||
                                'No se pudo actualizar el usuario.';
                ...
                eliminarUsuario() {
                    ...
                    fetch('/tecnologias/usuarios/' + encodeURIComponent(login), {
                        ...
                        .then(data => {
                            ...
                            if (this.usuarioSeleccionado && this.usuarioSeleccionado.login ===
                                login) {
